...
def config_parser():
    ...
    parser.add_argument('--config', is_config_file=True, default='./configs/amx.txt' ,
...
def val():
    ...
    if args.render:
        ...
        with torch.no_grad():
            ...
            rgbs, _ = render_path(render_poses, hwf, K, args.chunk, render_kwargs_test, gt_imgs=images_render, savedir=testsavedir, render_factor=args.render_factor)
    ...
    if True:
        with torch.no_grad():
            i_batch = 0
            batch = rays_rgb_val[i_batch:i_batch+N_rand] # [B, 2+1, 3*?]
            batch = torch.transpose(batch, 0, 1)
            batch_rays, target_s = batch[:2], batch[2]

            i_batch += N_rand
            if i_batch >= rays_rgb_val.shape[0]:
        ...
        print(f"[Val] Loss: {loss.item()}  PSNR: {psnr.item()}")
